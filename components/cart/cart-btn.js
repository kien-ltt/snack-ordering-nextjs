import { useContext, useEffect, useState } from 'react';
import CartContext from '../../store/cart-context';

export default function CartBtn(props) {
  const [animateCartBtn, setAnimateCartBtn] = useState(false);
  const { items } = useContext(CartContext);

  const numCartItems = items.reduce((curNumber, item) => {
    return curNumber + item.amount;
  }, 0);

  useEffect(() => {
    if (items.length === 0) {
      return;
    }

    setAnimateCartBtn(true);
    const timer = setTimeout(() => {
      setAnimateCartBtn(false);
    }, 300);

    return () => {
      clearTimeout(timer);
    };
  }, [items]);

  const cartBtnToggledHandler = () => {
    setAnimateCartBtn(true);
    props.onToggleCartBtn();
  };

  return (
    <button
      onClick={cartBtnToggledHandler}
      onAnimationEnd={() => {
        setAnimateCartBtn(false);
      }}
      className={`relative group p-3 bg-orange-100 rounded-xl shadow-sm hover:bg-yellow-400 hover:shadow-md transition-all duration-200 ${
        animateCartBtn && 'animate-bump'
      }`}
    >
      <svg
        xmlns='http://www.w3.org/2000/svg'
        fill='none'
        viewBox='0 0 24 24'
        strokeWidth={1.5}
        stroke='currentColor'
        className='w-6 h-6 text-yellow-500 group-hover:text-white'
      >
        <path
          strokeLinecap='round'
          strokeLinejoin='round'
          d='M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 00-3 3h15.75m-12.75-3h11.218c1.121-2.3 2.1-4.684 2.924-7.138a60.114 60.114 0 00-16.536-1.84M7.5 14.25L5.106 5.272M6 20.25a.75.75 0 11-1.5 0 .75.75 0 011.5 0zm12.75 0a.75.75 0 11-1.5 0 .75.75 0 011.5 0z'
        />
      </svg>
      <div className='absolute w-6 h-6 -top-2 -right-2 flex justify-center items-center text-white text-xs font-semibold bg-green-500 rounded-lg'>
        {numCartItems}
      </div>
    </button>
  );
}
